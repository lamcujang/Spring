...
public class TaxDeclarationExciseResource {

    private final TaxDeclarationExciseService service;

    @GetMapping("/{id}")
    public ResponseEntity<GlobalReponse> findById(@PathVariable("id") final Integer id) {
        log.info("*** TaxDeclarationExciseResourceDto, controller; fetch product by id *");
        return ResponseEntity.ok(this.service.findById(id));
    }

    @GetMapping("/findAll")
    public ResponseEntity<GlobalReponsePagination> findAll(@ModelAttribute TaxDeclarationExciseQueryRequest request) {
        log.info("*** TaxDeclarationExciseResourceDto List, controller; fetch all product *");
