...
import com.dbiz.app.proxyclient.business.user.service.LeaveApplicationClientService;
...
@RestController
@RequestMapping("/api/v1/leaveApplications")
@RequiredArgsConstructor
public class LeaveApplicationController {
    private final LeaveApplicationClientService clientService;

    @GetMapping
    public ResponseEntity<GlobalReponsePagination> findAll(@SpringQueryMap LeaveApplicationRequest request) {
        return ResponseEntity.ok(clientService.findAll(request)).getBody();
    }

    @PostMapping
    public ResponseEntity<GlobalReponse> save(@RequestBody LeaveApplicationDto request) {
