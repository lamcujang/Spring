...
@FeignClient(name = "REPORT-SERVICE", contextId = "individualIndustryClientService", path = "/report-service/api/v1/individualIndustry", decode404 = true
, configuration = FeignClientConfig.class )
public interface IndividualIndustryClientService {

    @GetMapping("/{id}")
    GlobalReponse findById(@PathVariable(name = "id") Integer id);

    @GetMapping("/findAll")
    GlobalReponsePagination findAll(@SpringQueryMap IndividualIndustryQueryRequest request);
