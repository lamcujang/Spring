...
public class NumericFieldValidation implements ValidationStrategy {
    ...
    public Boolean validate(Row row,
        ...
        //log.info("Number validated for row: {}", row.getRowNum() + 1);
        ...
        for (Map.Entry<Integer, String> entry : indexDataType.entrySet()) {
            ...
            // if dataType:Num && ( Req:Y && !Num ) || ( Req:N && !Blank && !Num )
            boolean isTypeNum = dataType.equalsIgnoreCase("numeric");
            ...
            boolean isNotNum = cell.getCellType() != CellType.NUMERIC || DateUtil.isCellDateFormatted(cell);
            if (isTypeNum && ( isRequired || hasValue ) && isNotNum) {
                String errorMessage = messageSource.getMessage(
                        "file.validation.numeric",
